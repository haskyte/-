...
#import "MJRefresh.h"
@interface SeekTableViewController ()
...
@implementation SeekTableViewController
...
- (void)footRefresh{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        NSString *url = [NSString stringWithFormat:@"https://chanyouji.com/api/search/trips.json?q=%@&page=%ld",self.searchText,self.page];
...
- (void)getNetWorkRequest{
    NSString *url = [NSString stringWithFormat:@"https://chanyouji.com/api/search/trips.json?q=%@&page=1",self.searchText];
    [[NetWorkManager shareWithManager]getDataWithURL:url Method:@"GET" Parameters:nil Kind:50 OrContainChinese:NO success:^(id obj) {
        [self getDataSuccessWithObject:obj];
    } fail:^{
        [self getField];
...
// 请求失败
- (void)getField{
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"没有找到你搜索的内容，或者网络错误" message:nil delegate:self cancelButtonTitle:@"好哒" otherButtonTitles:nil];
    [alertView show];
}
