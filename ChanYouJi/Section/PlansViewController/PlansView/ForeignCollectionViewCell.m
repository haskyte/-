...
@implementation ForeignCollectionViewCell
- (void)dealloc {
    self.image = nil;
    self.name_zh_cn = nil;
    self.name_en = nil;
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [self.image addSubview:self.name_zh_cn];
        [self.image addSubview:self.name_en];
        [self.image addSubview:self.poi_count];
    }
    return self;
}
...
- (void)setForeignModel:(ForeignModel *)foreignModel {
    ...
    self.poi_count.text = [NSString stringWithFormat:@"旅行地%ld",foreignModel.poi_count];
